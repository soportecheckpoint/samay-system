...
function Divider({ children }: { children?: ReactNode }) {
  if (!children) {
    return <div className="my-4 h-px bg-gradient-to-r from-transparent via-slate-300/40 to-transparent" />;
  }
  return (
...
export function HardwareDrawer({ open, marker, details, actions, onClose }: HardwareDrawerProps) {
  const status: MarkerStatus | undefined = details?.status;
  const StatusIcon = status ? STATUS_ICON_MAP[status.connection] : HelpCircle;
  const statusStyle = status ? STATUS_STYLE[status.connection] : STATUS_STYLE.unknown;
    ...
    <>
      ...
      >
        <div className="flex h-full flex-col overflow-hidden">
          <header className="border-b border-slate-300/60 bg-gradient-to-br from-white/40 to-transparent px-5 py-5">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                <h2 className="text-xl font-bold text-slate-800 tracking-tight">{marker.label}</h2>
                {marker.description && (
                  <p className="mt-2 text-xs text-slate-600 leading-relaxed">{marker.description}</p>
                )}
              </div>
              <button
                type="button"
          ...
          <div className="flex-1 overflow-y-auto px-5 py-5 scrollbar-thin scrollbar-thumb-slate-300/40 scrollbar-track-transparent">
            <section className={`rounded-2xl border-2 p-4 transition-all duration-300 ${statusStyle}`}>
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1">
                  <p className="text-[10px] uppercase tracking-[0.3em] opacity-60">Estado del equipo</p>
                  <p className="mt-0.5 text-sm font-semibold">{status?.label ?? 'Sin datos'}</p>
                </div>
              </div>
              {status?.helper && (
                <p className="mt-3 text-xs opacity-70 leading-relaxed">{status.helper}</p>
              )}
            </section>
              ...
              {details?.meta?.length === 0 && (
                <div className="flex items-start gap-3 rounded-xl border border-slate-200/60 bg-gradient-to-br from-white/60 to-slate-50/40 p-3.5 text-xs text-slate-600">
                  ...
                  <p className="leading-relaxed">
                    Conecta este marcador a un módulo o dispositivo para ver métricas en tiempo real.
                  </p>
                </div>
              )}
            </section>
            ...
            <div className="space-y-2.5">
              {actions.map((action) => (
                ...
                >
                  <div className="flex h-6 w-6 flex-shrink-0 items-center justify-center rounded-full bg-white/60">
                    {action.tone === 'primary' && <RotateCw className="h-3.5 w-3.5" strokeWidth={2.5} />}
                    {action.tone === 'danger' && <AlertCircle className="h-3.5 w-3.5" strokeWidth={2.5} />}
                  ...
                  <div className="flex-1 pt-0.5">
                    <span className="block leading-tight">
                      {action.requireHold ? '' : ''}{action.label}
                    </span>
                    {action.description && (
                      <span className="mt-1.5 block text-[10px] opacity-70 leading-relaxed">
                        {action.requireHold ? 'Mantén presionado para activar. ' : ''}{action.description}
                      </span>
                    )}
                  </div>
              ...
              {actions.length === 0 && (
                <div className="flex items-start gap-3 rounded-xl border border-slate-200/60 bg-gradient-to-br from-white/60 to-slate-50/40 p-3.5 text-xs text-slate-600">
                  <Info className="h-4 w-4 mt-0.5 flex-shrink-0 text-slate-400" />
                  <p className="leading-relaxed">No hay acciones directas para este marcador.</p>
                </div>
              )}
            </div>
          </div>
