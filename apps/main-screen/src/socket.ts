...
const applyTabletState = (tablet?: TabletStatePayload) => {
  ...
  useTabletSyncStore.getState().hydrate({
    ...
    photoData:
      typeof tablet.photoData === "undefined"
        ? undefined
        : (tablet.photoData ?? null),
    composedImage:
      typeof tablet.composedImage === "undefined"
        ? undefined
        : (tablet.composedImage ?? null),
    composedImagePath:
      typeof tablet.composedImagePath === "undefined"
        ? undefined
        : (tablet.composedImagePath ?? null),
    composedImageUrl:
      typeof tablet.composedImageUrl === "undefined"
        ? undefined
        : (tablet.composedImageUrl ?? null),
  });

  const trimmedMessage =
    typeof tablet.selectedMessage === "string"
      ? tablet.selectedMessage.trim()
      : "";

  if (trimmedMessage) {
    useMessageStore
      .getState()
      .setMessage(trimmedMessage, tablet.sessionId ?? "");
  }
...
export const useSocket = () => {
  useEffect(() => {
    if (!socket) {
      console.log(
        "[MAIN-SCREEN] Initializing socket connection to:",
        SERVER_URL,
      );
      ...
      socket.on("game:reset", () => {
        ...
        useTimerStore
          .getState()
          .hydrate({ total: 0, elapsed: 0, remaining: 0, status: "waiting" });
        useVictoryStore.getState().reset();
      ...
      socket.on("tablet:message-selected", (data: { messageText: string }) => {
        if (typeof data?.messageText === "string") {
          ...
          useTabletSyncStore
            .getState()
            .hydrate({ selectedMessage: data.messageText });
        }
      });
      ...
      socket.on(
        ...
        (data: {
          ...
          useTabletSyncStore.getState().hydrate({
            ...
            photoData:
              typeof data?.photoData === "undefined"
                ? undefined
                : (data.photoData ?? null),
            composedImage:
              typeof data?.composedImage === "undefined"
                ? undefined
                : (data.composedImage ?? null),
            composedImagePath:
              typeof data?.composedImagePath === "undefined"
                ? undefined
                : (data.composedImagePath ?? null),
            composedImageUrl:
              typeof data?.composedImageUrl === "undefined"
                ? undefined
                : (data.composedImageUrl ?? null),
          });
        },
      );
      ...
      socket.on("tablet:view-change", (data: { viewId?: string }) => {
        const viewId =
          typeof data?.viewId === "string" && data.viewId
            ? data.viewId
            : "camera-preview";
      ...
      socket.on(
        "game:victory",
        (data: { message: string; finalTime: number }) => {
      ...
      socket.on("tablet:mirror", (data: TabletMirrorPayload) => {
        ...
        const contentRecord = (mirrorPayload.content ?? {}) as Record<
          string,
          unknown
        >;
        ...
        if (
          typeof contentRecord.photoData === "string" ||
          contentRecord.photoData === null
        ) {
          syncSnapshot.photoData =
            (contentRecord.photoData as string | null) ?? null;
        }
      ...
      socket.on(
        "main-screen:previous-message",
        (data: PreviousMessagePayload) => {
          const message =
      ...
      socket.on("totem:view-change", (data: { view: string }) => {
        ...
        useTotemStore.getState().setView(view);
      });
    }
