...
import { useTabletMirrorStore, useTabletSyncStore, useWinOverlayStore } from '../store';
...
export const TabletStateImage: React.FC = () => {
  ...
  const { currentView, frameMessage, photoData, feedbackText } = useTabletSyncStore();
  const { imageSrc: winImageSrc, variant: winVariant } = useWinOverlayStore();

  const mirrorMessage = useMemo(() => {
    const raw = content ? (content as Record<string, unknown>)['frameMessage'] : undefined;
    const candidate = typeof raw === 'string' ? raw : undefined;
    return typeof candidate === 'string' ? candidate : '';
  }, [content]);

  const mirrorFeedback = useMemo(() => {
    const raw = content ? (content as Record<string, unknown>)['feedbackText'] : undefined;
    const candidate = typeof raw === 'string' ? raw : undefined;
    return typeof candidate === 'string' ? candidate : '';
  }, [content]);

  const mirrorPhoto = useMemo(() => {
    const raw = content ? (content as Record<string, unknown>)['photoData'] : undefined;
    const candidate = typeof raw === 'string' ? raw : undefined;
  ...
  if (resolvedView === 'photo-preview') {
    ...
    return (
      <div className="flex w-full max-w-xl flex-col gap-8">
        <div className="rounded-[40px] bg-white px-10 py-8 text-center">
        ...
        {displayPhoto && (
          <div className="overflow-hidden rounded-[40px] bg-white">
            <img src={displayPhoto} alt="Foto del equipo" className="w-full object-contain" />
          </div>
        )}
      </div>
    );
